...
from .tuya_cloud_const import DEVICE_TYPE_CLIMATES
from .tuya_cloud_entity import TuyaClimateEntity
from .tuya_cloud_models import HubConfigEntry, RuntimeData
...
async def async_setup_entry(
    ...
    for data in climates_data:
        try:
            with open("/config/debug_tuya.txt", "a") as f:
                f.write(f"Instantiating TuyaClimate for data: {data}\n")
            entity = TuyaClimate(data, runtime_data)
            active_entities.append(entity)
        except Exception as e:
            with open("/config/debug_tuya.txt", "a") as f:
                f.write(f"Error instantiating TuyaClimate: {e}\n")
    ...
    if active_entities:
        ...
        with open("/config/debug_tuya.txt", "a") as f:
            f.write(f"Calling async_add_entities with {len(active_entities)} entities\n")
        try:
            async_add_entities(active_entities)
            with open("/config/debug_tuya.txt", "a") as f:
                f.write(f"async_add_entities called successfully\n")
        except Exception as e:
            with open("/config/debug_tuya.txt", "a") as f:
                f.write(f"Error in async_add_entities: {e}\n")


class TuyaClimate(TuyaClimateEntity, CoordinatorEntity, ClimateEntity):
    """Tuya Climate Entity."""
    ...
    def __init__(self, config_data: dict[str, Any], runtime_data: RuntimeData) -> None:
        ...
        coordinator = runtime_data.climate_coordinator or runtime_data.coordinator
