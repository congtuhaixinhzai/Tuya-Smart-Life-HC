...
class TuyaIrFan(CoordinatorEntity[TuyaSmartLifeCoordinator], FanEntity):
    ...
    def __init__(
        ...
        if _find_ir_action(remote.actions, IR_SWING_ALIASES):
    ...
    def available(self) -> bool:
        ...
        if not remote:
            return False
        return (
            self.runtime.local.has_ir_local_path(remote.remote_id, remote.hub_dev_id)
        )
    ...
    async def async_oscillate(self, oscillating: bool) -> None:
        if oscillating == self._oscillating:
            return
        action = self._find(IR_SWING_ALIASES)
        await self._send(action)
