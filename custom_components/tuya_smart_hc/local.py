...
class TuyaLocalRuntime:
    ...
    def __init__(self, hass: HomeAssistant) -> None:
        ...
        self._state_stream_pause_until: dict[str, float] = {}
        self._local_connected: dict[str, bool] = {}
        self._last_force_scan = 0.0
        self.mqtt_events: dict[str, dict[str, Any]] = {}
    ...
    def update_devices(self, devices: list[TuyaDeviceDescription]) -> None:
        ...
        temp_dict = {d.dev_id: d for d in devices}

        # Preserve broadcast-discovered IP/version across cloud metadata refreshes.
        for device in devices:
            if _is_ir_virtual_child(device, temp_dict) or _looks_like_ir_hub(device):
                continue
        ...
        self._state_stream_pause_until = {
            dev_id: pause_until
            for dev_id, pause_until in self._state_stream_pause_until.items()
            if dev_id in next_devices
        }
        self._local_connected = {
            dev_id: connected
            for dev_id, connected in self._local_connected.items()
            if dev_id in next_devices
        }
        self._tinytuya_devices.clear()
    ...
    def apply_mqtt_dps(
        ...
        if not device:
            return
        self.mqtt_events[dev_id] = event or {"data": {"dps": dps}}
        changed = _apply_device_dps(device, dps)
        callback_dps = changed or {str(key): value for key, value in dps.items()}
        _LOGGER.debug(
            "Tuya cloud MQTT updated %s changed=%s dps=%s",
            dev_id,
            bool(changed),
            callback_dps,
        )
        for listener in list(self._state_callbacks):
            self.hass.loop.call_soon_threadsafe(listener, dev_id, callback_dps)

    def apply_mqtt_event(self, dev_id: str, event: dict[str, Any]) -> None:
        if dev_id not in self.devices:
            return
        self.mqtt_events[dev_id] = event
        for listener in list(self._state_callbacks):
            self.hass.loop.call_soon_threadsafe(listener, dev_id, {})
            ...
            try:
                ...
                if pause_for > 0:
                    _close_tinytuya_device(
                        self._stream_tinytuya_devices.pop(dev_id, None)
                    )
                    self._last_heartbeat.pop(dev_id, None)
                    self._stream_synced.discard(dev_id)
    ...
    def _receive_state_stream(self, dev_id: str) -> list[dict[str, Any]]:
        ...
        if isinstance(payload, dict):
            if not _response_has_error(payload):
                self._set_local_connected(dev_id, True)
            _LOGGER.debug(
    ...
    def _sync_state_stream(self, dev_id: str, stream_device: Any) -> list[dict[str, Any]]:
        ...
        if hasattr(stream_device, "updatedps"):
            try:
                ...
                self._set_local_connected(dev_id, False)
                _LOGGER.debug(
                ...
                if isinstance(response, dict):
                    self._set_local_connected(
                        dev_id,
                        not _response_has_error(response),
                    )
                    _LOGGER.debug(
        ...
        for endpoint in endpoints:
            ...
            try:
                ...
                self._set_local_connected(endpoint.dev_id, False)
                _LOGGER.debug(
                ...
                continue
            if isinstance(response, dict):
                connected = not _response_has_error(response)
                self._set_local_connected(endpoint.dev_id, connected)
                if endpoint.dev_id != dev_id:
                    self._set_local_connected(dev_id, connected)
    ...
    def _handle_state_stream_payload(
        ...
        if not target:
            ...
            return
        self._set_local_connected(stream_dev_id, True)
        self._set_local_connected(target.dev_id, True)
    ...
    def switch_button_dps(self) -> list[tuple[TuyaDeviceDescription, str, bool, str]]:
        ...
        for device in self.devices.values():
            if (
                not device.local_controllable
                or device.is_hub
                or _looks_like_ir_hub(device)
                or _is_ir_virtual_child(device, self.devices)
            ):
                continue
    ...
    def environment_sensor_dps(
        ...
        for device in self.devices.values():
            if not device.local_controllable:
                continue
            for dp_id, value in device.dps.items():
                raw_items = _raw_environment_sensor_dps(device, dp_id, value)
                if raw_items:
                ...
                if not kind:
                    continue
                native_value = _normalize_environment_sensor_value(
                    value,
                    kind,
                    device,
                    dp_id,
    ...
    def cover_control_dps(self) -> list[tuple[TuyaDeviceDescription, str, str]]:
        ...
        for device in self.devices.values():
            ...
            if not dp_id:
                continue
            items.append((device, dp_id, _cover_label(device, dp_id)))
        return items

    def cover_travel_time_dps(
        self,
        ...
        for device in self.devices.values():
            ...
            if not _cover_control_dp(device):
                continue
            dp_id = _cover_travel_time_dp(device)
            if not dp_id:
                continue
            value = _normalize_schema_value(device.dps.get(dp_id), device, dp_id)
            items.append((device, dp_id, value, _cover_travel_time_label(device, dp_id)))
        return items

    def child_lock_dps(
        self,
    ) -> list[tuple[TuyaDeviceDescription, str, bool | None, str]]:
        items: list[tuple[TuyaDeviceDescription, str, bool | None, str]] = []
        for device in self.devices.values():
            if (
                not device.local_controllable
                or device.is_hub
                ...
                continue
            for dp_id in device.dp_schema:
                if not _is_child_lock_dp(device, str(dp_id)):
                    continue
                current = device.dps.get(str(dp_id))
                items.append(
                    (
                        device,
                        str(dp_id),
                        current if isinstance(current, bool) else None,
                        _child_lock_label(device, str(dp_id)),
                    )
                )
        return items
    ...
    def has_local_connection(self, device: TuyaDeviceDescription | None) -> bool:
        if not device:
            return False
        if (
            device.is_child
            and self._local_connected.get(device.dev_id) is not True
            and _is_ir_virtual_child(device, self.devices)
        ):
            parent = self.devices.get(device.parent_dev_id or "")
            return self._local_connected.get(parent.dev_id if parent else "") is True
        return self._local_connected.get(device.dev_id) is True


    def local_connection_attributes(
        self,
        device: TuyaDeviceDescription | None,
    ) -> dict[str, Any]:
        if not device:
            return {}
        parent = self.devices.get(device.parent_dev_id or "") if device.is_child else None
        attrs: dict[str, Any] = {
            "ip": device.ip,
            "protocol_version": device.protocol_version,
            "local_path_available": self.has_local_path(device),
            "remote_online": device.online,
        }
        local_ip = self.local_ip_for_device(device)
        if local_ip:
            attrs["local_ip"] = local_ip
        if device.is_child:
            attrs.update(
                {
                    "parent_dev_id": device.parent_dev_id,
                    "parent_name": parent.name if parent else None,
                    "parent_protocol_version": parent.protocol_version if parent else None,
                    "parent_local_connected": self.has_local_connection(parent),
                }
            )
        return attrs

    def local_ip_for_device(self, device: TuyaDeviceDescription | None) -> str | None:
        if not device:
            return None
        if device.is_child:
            return None
    ...
    def wifi_ip_devices(self) -> list[TuyaDeviceDescription]:
        return [
            device
            for device in self.devices.values()
            if not device.is_child and bool(device.local_key or device.ip)
        ]
    ...
    def _status(self, dev_id: str) -> dict[str, Any]:
        ...
        if _is_key_or_version_error(response):
            ...
            if fallback is not NO_FALLBACK_RESPONSE:
                response = fallback
        if isinstance(response, dict):
            self._set_local_connected(dev_id, not _response_has_error(response))
            return response
        self._set_local_connected(dev_id, response is not None)
        return {}
    ...
    def _set_dp(self, dev_id: str, dp_id: int, value: Any) -> Any:
        ...
        if stream_response is not NO_FALLBACK_RESPONSE:
            self._update_command_local_connection(dev_id, stream_response)
        ...
        try:
            ...
            if stream_dev_id and stream_dev_id != dev_id:
                self._set_local_connected(stream_dev_id, False)
            raise
        if _is_key_or_version_error(response):
            fallback = self._try_child_protocol_fallback(dev_id, dp_id, value)
            if fallback is not NO_FALLBACK_RESPONSE:
                self._update_command_local_connection(dev_id, fallback)
                return fallback
        self._update_command_local_connection(dev_id, response)
        return response
    ...
    def _publish_ir_action(self, action: TuyaIrAction | str) -> Any:
        ...
        try:
            ...
            self._set_ir_action_local_connected(action, False)
            raise
        ...
        if _is_key_or_version_error(response) and _is_dp201_ir_action(action):
            _LOGGER.warning(
                ...
                "leaving the DP201 IR command failed so callers can retry or surface "
                "the error: %s",
                action.hub_dev_id,
                action.action_name,
                response,
        ...
        if _is_unexpected_payload_error(response):
            ...
            self._set_ir_action_local_connected(action, True)
            return None
        self._set_ir_action_local_connected(
            action,
            not _response_has_error(response),
        )
        return response
    ...
    def _publish_ir_dps_once(self, action: TuyaIrAction) -> Any:
        target_dev_id = self._ir_publish_target_dev_id(action)
        device = self._tinytuya_device(target_dev_id)
        if not device:
            raise RuntimeError(
                f"IR target {target_dev_id} is missing local metadata or IP"
            )

        normalized_dps = _normalize_command_dps(action.action_dps)
        if _is_normalized_dp201_command(normalized_dps):
            _close_tinytuya_device(self._tinytuya_devices.pop(target_dev_id, None))
            device = self._tinytuya_device(target_dev_id)
            if not device:
                raise RuntimeError(
                    f"IR target {target_dev_id} is missing local metadata or IP"
        ...
        _LOGGER.debug(
            "Publishing Tuya IR action %s to target %s via hub %s for remote %s with DPS %s",
            action.action_name,
            target_dev_id,
            action.hub_dev_id,
            action.remote_id,
        ...
        if _is_normalized_dp201_command(normalized_dps):
            ir_payload = _normalized_dp201_payload(normalized_dps)
            if hasattr(device, "set_value"):
                return _call_tinytuya_writer(
                    device.set_value,
                    201,
    ...
    def _ir_publish_target_dev_id(self, action: TuyaIrAction) -> str:
        if _is_dp201_ir_action(action):
            # DP201 keydata payloads are hub commands. Sending them through the
            # virtual remote child can succeed locally without emitting IR.
            return action.hub_dev_id
        remote = self.devices.get(action.remote_id)
        if remote and remote.is_child and IR_SEND_DP_ID in remote.dps:
            return remote.dev_id
        return action.hub_dev_id
    ...
    def _send_ir_payload(self, hub_dev_id: str, payload: dict[str, Any]) -> Any:
        ...
        try:
            ...
            self._set_local_connected(hub.dev_id, False)
            raise
        if _is_key_or_version_error(response) or _is_unexpected_payload_error(response):
            _LOGGER.debug(
        ...
        if _is_key_or_version_error(response) or _is_unexpected_payload_error(response):
            ...
            self._set_local_connected(hub.dev_id, True)
            return None
        self._set_local_connected(hub.dev_id, not _response_has_error(response))
        return response

    def _set_local_connected(self, dev_id: str | None, connected: bool) -> None:
        if not dev_id or dev_id not in self.devices:
            return
        previous = self._local_connected.get(dev_id)
        if previous is connected:
            return
        self._local_connected[dev_id] = connected
        _LOGGER.debug("Tuya local connection %s connected=%s", dev_id, connected)
        self._notify_metadata_listeners()
        for listener in list(self._state_callbacks):
            self.hass.loop.call_soon_threadsafe(listener, dev_id, {})

    def _set_stream_local_connected(self, stream_dev_id: str, connected: bool) -> None:
        self._set_local_connected(stream_dev_id, connected)
        for device in self.devices.values():
            if device.parent_dev_id == stream_dev_id:
                self._set_local_connected(device.dev_id, connected)

    def _update_command_local_connection(self, dev_id: str, response: Any) -> None:
        connected = not _response_has_error(response)
        self._set_local_connected(dev_id, connected)
        stream_dev_id = self._command_stream_device_id(dev_id)
        if stream_dev_id and stream_dev_id != dev_id:
    ...
    def _set_ir_action_local_connected(
        self,
        action: TuyaIrAction,
        connected: bool,
    ) -> None:
        target_dev_id = self._ir_publish_target_dev_id(action)
        self._set_local_connected(action.hub_dev_id, connected)
        self._set_local_connected(target_dev_id, connected)
    ...
    def _release_state_stream_for_command(self, dev_id: str) -> None:
        ...
        if not stream_dev_id:
            return
        self._pause_state_stream_for_command(stream_dev_id)
    ...
    def _pause_state_stream_for_command(self, stream_dev_id: str) -> None:
        pause_until = time.monotonic() + STATE_STREAM_COMMAND_PAUSE_SECONDS
        previous = self._state_stream_pause_until.get(stream_dev_id, 0.0)
        if pause_until > previous:
            self._state_stream_pause_until[stream_dev_id] = pause_until

    def _state_stream_pause_remaining(self, stream_dev_id: str) -> float:
        pause_until = self._state_stream_pause_until.get(stream_dev_id, 0.0)
        remaining = pause_until - time.monotonic()
        if remaining <= 0:
            self._state_stream_pause_until.pop(stream_dev_id, None)
            return 0.0
        return remaining
...
def _prefer_dp201_ir_actions(actions: list[TuyaIrAction]) -> list[TuyaIrAction]:
    dp201_actions = [action for action in actions if _is_dp201_ir_action(action)]
    return dp201_actions or actions
...
def _ir_remote_kind(actions: list[TuyaIrAction]) -> str:
    ...
    if _is_ir_climate_remote(actions):
        return "climate"
    raw_category = " ".join(action.category or "" for action in actions).lower()
    category = f"{raw_category} {_ascii_fold(raw_category).lower()}"
    ...
    if any(marker in category for marker in IR_LIGHT_CATEGORY_MARKERS):
        return "light"
    if _is_scene_only_remote(actions):
        return "button"
    raw_text = " ".join(
        [
    ...
    text = f"{raw_text} {_ascii_fold(raw_text).lower()}"
...
def _is_scene_only_remote(actions: list[TuyaIrAction]) -> bool:
    if not actions:
        return False
    for action in actions:
        action_name = _ascii_fold(action.action_name).strip().lower()
        if not action_name.startswith("scene"):
            return False
        values = {str(value).lower() for value in action.action_dps.values()}
        if values and values != {"scene"}:
            return False
    return True
...
def _is_normalized_dp201_command(dps: dict[Any, Any]) -> bool:
    return {str(dp_id) for dp_id in dps} == {IR_SEND_DP_ID}


def _normalized_dp201_payload(dps: dict[Any, Any]) -> Any:
    if IR_SEND_DP_ID in dps:
        return dps[IR_SEND_DP_ID]
    return dps[int(IR_SEND_DP_ID)]
...
def _environment_sensor_kind(device: TuyaDeviceDescription, dp_id: Any) -> str | None:
    ...
    schema_mode = str(schema.get("mode") or "").lower()
    if schema and (schema_type not in ("", "value") or schema_mode not in ("", "ro")):
        return None

    text = _dp_text(device, str(dp_id))
    if any(marker in text for marker in SCHEMA_SENSOR_IGNORED_MARKERS):
        return None
    if any(marker in text for marker in TEMPERATURE_DP_MARKERS):
        return "temperature"
    if any(marker in text for marker in HUMIDITY_DP_MARKERS):
        return "humidity"
    if any(marker in text for marker in VOLTAGE_DP_MARKERS):
        return "voltage"
    if any(marker in text for marker in POWER_DP_MARKERS):
        return "power"
    if any(marker in text for marker in CURRENT_DP_MARKERS):
        return "current"
    if any(marker in text for marker in ENERGY_DP_MARKERS):
        return "energy"
    return None


def _raw_environment_sensor_dps(
    device: TuyaDeviceDescription,
    dp_id: Any,
    value: Any,
) -> list[tuple[TuyaDeviceDescription, str, float, str, str]]:
    schema = device.dp_schema.get(str(dp_id), {})
    if str(schema.get("type") or "").lower() != "raw":
        return []
    if not _is_phase_realtime_dp(device, dp_id):
        return []
    decoded = _decode_phase_realtime_value(value)
    if not decoded:
        return []
    phase = _phase_realtime_label(device, dp_id)
    voltage, current, power = decoded
    return [
        (device, str(dp_id), voltage, "voltage", f"{phase} Voltage"),
        (device, str(dp_id), current, "current", f"{phase} Current"),
        (device, str(dp_id), power, "power", f"{phase} Power"),
    ]


def _is_phase_realtime_dp(device: TuyaDeviceDescription, dp_id: Any) -> bool:
    text = _dp_text(device, str(dp_id))
...
def _phase_realtime_label(device: TuyaDeviceDescription, dp_id: Any) -> str:
    ...
    if code.startswith("phase_") and len(code) >= len("phase_a"):
        return f"Phase {code[len('phase_')].upper()}"
...
def _decode_phase_realtime_value(value: Any) -> tuple[float, float, float] | None:
    ...
    if len(raw) < 8:
        return None
    voltage = int.from_bytes(raw[0:2], "big") / 10
    current = int.from_bytes(raw[2:5], "big") / 1000
    power = float(int.from_bytes(raw[5:8], "big"))
    return voltage, current, power


def _normalize_environment_sensor_value(
    value: Any,
    kind: str,
    device: TuyaDeviceDescription,
    dp_id: Any,
    ...
    if kind in ("temperature", "humidity") and abs(number) >= 100:
        return number / 10
    return number
...
def _cover_control_dp(device: TuyaDeviceDescription) -> str | None:
    for dp_id, schema in device.dp_schema.items():
        if str(schema.get("type") or "").lower() != "enum":
            continue
        if str(schema.get("mode") or "").lower() not in ("", "rw", "wr"):
            continue
        prop = schema.get("property") if isinstance(schema, dict) else None
        values = prop.get("range") if isinstance(prop, dict) else None
        if not isinstance(values, list):
            continue
        commands = {str(value).lower() for value in values}
        if not {"open", "stop", "close"}.issubset(commands):
            continue
        text = _dp_text(device, str(dp_id))
        if "control" in text or "curtain" in text or "gate" in text or "cover" in text:
            return str(dp_id)
    return None


def _cover_label(device: TuyaDeviceDescription, dp_id: str) -> str:
    schema = device.dp_schema.get(str(dp_id), {})
    code = str(schema.get("code") or "").strip()
    if code and code not in {"control"}:
        return _title_from_code(code)
    return "Cửa cuốn"


def _cover_travel_time_dp(device: TuyaDeviceDescription) -> str | None:
    for dp_id, schema in device.dp_schema.items():
        if str(schema.get("type") or "").lower() != "value":
            continue
        if str(schema.get("mode") or "").lower() not in ("", "rw", "wr"):
            continue
        text = _dp_text(device, str(dp_id))
        raw_text = _dp_raw_text(device, str(dp_id))
        if any(marker in text or marker in raw_text for marker in COVER_TRAVEL_TIME_MARKERS):
            return str(dp_id)
    return None


def _cover_travel_time_label(device: TuyaDeviceDescription, dp_id: str) -> str:
    name = device.dp_names.get(str(dp_id), "").strip()
    if name:
        return name
...
def _is_ir_virtual_child(
    device: TuyaDeviceDescription,
    devices: dict[str, TuyaDeviceDescription],
) -> bool:
    if not device.is_child:
        return False
    parent = devices.get(device.parent_dev_id or "")
    if parent and _looks_like_ir_hub(parent):
        return True
    if IR_SEND_DP_ID in device.dps or IR_RECEIVE_DP_ID in device.dps:
        return True
    return device.category in {"qt"} or device.category_code in {"wf_qt"}
...
def _dp_text(device: TuyaDeviceDescription, dp_id: str) -> str:
    schema = device.dp_schema.get(str(dp_id), {})
    prop = schema.get("property") if isinstance(schema, dict) else {}
    unit = prop.get("unit") if isinstance(prop, dict) else ""
    return _ascii_fold(
        " ".join(
            (
                str(dp_id),
                device.dp_names.get(str(dp_id), ""),
                str(schema.get("code") or ""),
...
def _dp_raw_text(device: TuyaDeviceDescription, dp_id: str) -> str:
    schema = device.dp_schema.get(str(dp_id), {})
    prop = schema.get("property") if isinstance(schema, dict) else {}
    unit = prop.get("unit") if isinstance(prop, dict) else ""
    return " ".join(
...
def _is_switch_button_dp(device: TuyaDeviceDescription, dp_id: str) -> bool:
    schema = device.dp_schema.get(str(dp_id), {})
    code = _ascii_fold(str(schema.get("code") or "")).strip().lower()
    schema_type = str(schema.get("type") or "").strip().lower()
    if code:
        return schema_type == "bool" and (
            code == "switch"
            or code.startswith("switch_")
            or code in {"on_off", "power", "power_switch", "switch_led"}
        )
...
def _is_child_lock_dp(device: TuyaDeviceDescription, dp_id: str) -> bool:
    schema = device.dp_schema.get(str(dp_id), {})
    if str(schema.get("type") or "").strip().lower() != "bool":
        return False
    if str(schema.get("mode") or "").strip().lower() not in ("", "rw", "wr"):
        return False
    text = _dp_text(device, str(dp_id))
    raw_text = _dp_raw_text(device, str(dp_id))
    return any(marker in text or marker in raw_text for marker in CHILD_LOCK_MARKERS)


def _child_lock_label(device: TuyaDeviceDescription, dp_id: str) -> str:
    schema = device.dp_schema.get(str(dp_id), {})
    code = str(schema.get("code") or "").strip()
    if code:
        return _title_from_code(code)
