...
class TuyaDeviceDescription:
    ...
    dp_schema: dict[str, dict[str, Any]] = field(default_factory=dict)
    raw: dict[str, Any] = field(default_factory=dict)
...
def dp_schema_from_raw(raw: dict[str, Any]) -> dict[str, dict[str, Any]]:
    product_info = raw.get("productInfo")
    if not isinstance(product_info, dict):
        return {}
    schema_info = product_info.get("schemaInfo")
    ...
    for item in schema:
        ...
        items[dp_id] = {
            ...
            "type": str(item.get("subType") or item.get("type") or ""),
...
def device_from_raw(
    ...
    dp_schema = dp_schema_from_raw(raw)
    for dp_id, schema in dp_schema.items():
        name = schema.get("name")
        if name and not dp_names.get(dp_id):
            dp_names[dp_id] = str(name)
