...
from .tuya_cloud_models import (
    RuntimeData,
    TuyaClimateData,
    TuyaSensorData
)
from .tuya_cloud_helpers import (
    valid_sensor_state,
    convert_temperature,
    convert_to_float,
)
...
class TuyaClimateEntity:
    ...
    def available(self) -> bool:
        ...
        if hasattr(self.coordinator, "is_available"):
    ...
    @property
    def _device_climate_data(self) -> TuyaClimateData:
        """Helper to get current climate data from coordinator."""
        if not self.coordinator or not hasattr(self.coordinator, "data") or not self.coordinator.data:
    ...
    def _check_cloud_api(self):
        if not hasattr(self.coordinator, "async_set_hvac_mode"):
            raise ValueError("You need to configure Cloud API in the integration options to control IR devices.")
    ...
    async def async_handle_set_fan_mode(self, fan_mode: str) -> None:
        """Set fan speed for the climate device via coordinator service."""
        self._check_cloud_api()
        if not self._custom_power_on and self.get_fan_power_on(self._current_fan_mode):
            await self.coordinator.async_turn_on_with_fan_mode(
                self._infrared_id, self._climate_id, fan_mode
            )
            return

        if self._custom_power_on and self.get_fan_power_on(self._current_fan_mode):
            await self._async_trigger_custom_power_on()
